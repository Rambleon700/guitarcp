...
function loadChord(name) {
  ...
  updateCurrentChordDisplay(name);
}

// Strum arrows + "now playing" chord name
function renderStrumArrows(pattern) {
  const container = document.getElementById("strum-arrows");
  if (!container) return;
  const strokes = pattern && pattern.length ? pattern : ["D"];
  container.innerHTML = strokes
    .map((s, i) => `<span class="strum-arrow" data-index="${i}">${s === "U" ? "↑" : "↓"}</span>`)
    .join("");
}

function highlightStrumStep(index) {
  const container = document.getElementById("strum-arrows");
  if (!container) return;
  container.querySelectorAll(".strum-arrow").forEach((el) => {
    el.classList.toggle("flash", Number(el.dataset.index) === index);
  });
}

function updateCurrentChordDisplay(name) {
  const label = document.getElementById("currentChordName");
...
window.highlightStrumStep = highlightStrumStep;
window.updateCurrentChordDisplay = updateCurrentChordDisplay;
