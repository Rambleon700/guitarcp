...
function playChord(chord) {
  ...
  var strumDelay = 0.02; // stagger notes slightly, like a real downstrum
  var noteDuration = 2.4;

  chord.frets.forEach(function (fret, i) {
    if (fret < 0) return;

    var midi = openMidi[i] + fret;
    var freq = 440 * Math.pow(2, (midi - 69) / 12);
    var startTime = now + i * strumDelay;
    ...
    var velocity = 0.55 + Math.random() * 0.08;
