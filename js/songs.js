  ...
  document.querySelectorAll(".practice-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      if (song) {
        ...
        // Sync tempo to this song's BPM, and reflect it in the slider UI
        if (song.bpm) {
          currentBPM = song.bpm;
          const bpmSlider = document.getElementById("bpmSlider");
          const bpmDisplay = document.getElementById("bpmDisplay");
          if (bpmSlider) bpmSlider.value = song.bpm;
          if (bpmDisplay) bpmDisplay.textContent = song.bpm;
        }

        console.log("Loaded for practice:", song.title, "@", song.bpm, "BPM");
      }
    });
  });
}
